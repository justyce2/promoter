...
if (!$token || $token !== $_SESSION['token']) {
    ...
    if (empty($error_msg)) {
		...
		if(isset($_COOKIE['ap_sponsor_tracking'])){$sponsor = $_COOKIE['ap_sponsor_tracking'];}else{$sponsor=0;}
			...
			//$insert_stmt = $mysqli->prepare("INSERT INTO ap_members (username, fullname, email, password, terms) VALUES (?, ?, ?, ?, ?)");
		//$insert_stmt->bind_param('sssss', $username, $fullname, $email, $hash, $terms);
		//$insert_stmt->execute();
		
	/**$sql = "INSERT INTO ap_members (fullname, username, email, password, terms, bank_name, account_name, account_number)
VALUES ('".$fullname."', '".$username."', '".$email."', '".$hash."', '".$terms."', '".$bank_name."', '".$account_name."', '".$account_number."')";
...
//$mysqli->close();
	//	die;
      ...
      if ($insert_stmt = $mysqli->prepare("INSERT INTO ap_members (fullname, username, email, password, terms, bank_name, account_name, account_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")) {
            
          // echo("reached");
          //  die;
            $insert_stmt->bind_param('ssssssss', $fullname, $username, $email, $hash, $terms, $bank_name,$account_name, $account_number );
