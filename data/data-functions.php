...
function profile_details($owner){
	...
	$get_profile = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT * FROM ap_members WHERE id=$owner"));
	$profile_name = $get_profile['fullname'];
	$profile_email = $get_profile['email'];
	$profile_username = $get_profile['username'];
	$bank_name = $get_profile['bank_name'];
	$account_name = $get_profile['account_name'];
	$account_number = $get_profile['account_number'];
	echo '
		<form method="post" action="data/update-user">
		<div><label class="control-label" for="">Full Name</label>
			<input type="text" class ="form-control" name="f" value="'.$profile_name.'"></div>
			<div><label class="control-label" for="">Email</label>
			
			<input type="text" name="e" class ="form-control" value="'.$profile_email.'"></div>
			<div><label class="control-label" for="">Bank Name</label>
			<input type="text" name="bank_name" class ="form-control" value="'.$bank_name.'"></div>
			<div><label class="control-label" for="">Account Name</label>
			<input type="text" name="account_name" class ="form-control" value="'.$account_name.'"></div>
			<div><label class="control-label" for="">Account Number</label>
			<input type="text" name="account_number" class ="form-control" value="'.$account_number.'"></div>
...
function get_attendee_name($attendee_id){
    	include('auth/db-connect.php');
    $name = '';
 $query = "SELECT * FROM tbl_attendee WHERE att_id=$attendee_id";
 if($result = $mysqli->query($query)){
			//$num_results = mysqli_num_rows($result);
			while($row = $result->fetch_array())
				{
...
function get_attendee_tickets($attendee_id){
    	include('auth/db-connect.php');
    $ticket = '';
 $query = "SELECT * FROM tbl_attendee WHERE att_id=$attendee_id";
 if($result = $mysqli->query($query)){
	...
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			...
			while($row = $result->fetch_array())
				...
				$currency_symbol = $money_format->getSymbol(\NumberFormatter::INTL_CURRENCY_SYMBOL);
				echo '<tr> 
						<td>'.get_attendee_name($row['attendee_id']). '</td>
						';
				echo ' 
						<td>'.get_attendee_tickets($row['attendee_id']). '</td>
						<td>'; 
							if($row['void']=='1'){ echo '<span class="red">-'.$money_format->formatCurrency($row['sale_amount'], $currency_symbol).' (Refunded)</span>'; } else { echo $money_format->formatCurrency($row['sale_amount'], $currency_symbol); }
						echo '</td>
						<td>'.$row['comission'].'%</td>
			...
			while($row = $result->fetch_array())
				...
				echo '<tr>
						...
						<td>';
							...
							if($row['payment_method']=='2'){echo 'Stripe';}
							if($row['payment_method']=='3'){echo 'Skrill';}
							if($row['payment_method']=='4'){echo 'Bank Transfer';}
							if($row['payment_method']=='5'){echo 'Check';}
						echo '</td>
						<td>'.$row['payment_email'].'</td>
						<td>'; 
	...
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			...
			while($row = $result->fetch_array())
				...
				if($row['wire']=='1'){echo '<option value="4">Bank Transfer</option>';}
...
function total_sales_period($start_date, $end_date, $affiliate_filter){
	...
	$start_date = $start_date.'000000';
		...
		$start_date = str_replace("-", "", $start_date);
		$st = "'AND datetime >'. $start_date";
	}
	if(!empty($end_date)){
	    
	  	$end_date = $end_date.'235959';  
	  	$end_date = str_replace("-", "", $end_date);
	  	$et= "'AND datetime >'. $end_date";
	}
$show = '';

	
	if(isset($affiliate_filter)){$show = ' AND affiliate_id='.$affiliate_filter.'';}
//	$get_tb = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT SUM(sale_amount) as total_sales FROM ap_earnings WHERE void!=1 $st  $et $show"));
	$get_tb = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT SUM(sale_amount) as total_sales FROM ap_earnings WHERE void!=1"));
	$tb = $get_tb['total_sales'];
	if($tb==''){$tb='0.00';}
	$money_format = new \NumberFormatter($locale, \NumberFormatter::CURRENCY); 
...
/* ===========================================
 ...
 function get_users_payouts(){
     ...
     $min_payout = $get_min['min_payout'];
   	$query = "SELECT * FROM ap_members WHERE admin_user='0' ORDER BY id ASC";
   	if($result = $mysqli->query($query)){
   	$num_results = mysqli_num_rows($result);
			while($row = $result->fetch_array())
				{
                    ...
                    $account_number = $row['account_number'];
                    	...
                    	$pm ='4';
                    	$ppe = $row['email'];
                    $get_requests= mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT SUM(amount) as pending FROM ap_payouts WHERE affiliate_id=$user_id AND status=0"));
                    ...
                    $amount  = $true_balance;
				  	...
				  	$sql = "INSERT INTO ap_payouts (affiliate_id, payment_method, payment_email, amount,  bn, an, rn, datetime)
VALUES ('".$user_id."', '".$pm."', '".$ppe."', '".$amount."', '".$bank_name."', '".$account_name."', '".$account_number."', '".$datetime."')";
...
/**$stmt = $mysqli->prepare("INSERT INTO ap_payouts (affiliate_id, payment_method, payment_email, amount,  bn, an, rn, datetime) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                        	$stmt->bind_param('ssssssss', $user_id, $pm, $ppe, $amount, $bank_name, $account_name, $account_number, $datetime);
                        	$stmt->execute();
                        	$stmt->close();  **/
	}
