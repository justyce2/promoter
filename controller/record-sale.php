...
//	$_SESSION['ap_sale_hit'] = '0';
	...
	//	$stmt = $mysqli->prepare("INSERT INTO ap_earnings (affiliate_id, product, comission, sale_amount, net_earnings, recurring, recurring_fee, datetime, stop_recurring, country, datetime, void, refund ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	//	$stmt->bind_param('sssssssssssss', $affiliate_id, $product, $comission, $sale_amount, $net_earnings, $recurring_period, $recurring_fee, $datetime, '0', 'NG', $datetime, '0', '0');
	//	$stmt->execute();
	...
	$attendee_id =	$_SESSION['attendee_id'];
			$sql = "INSERT INTO ap_earnings (affiliate_id, product, comission, sale_amount, net_earnings, recurring, recurring_fee, datetime, stop_recurring, country,last_reoccurance ,void, refund, attendee_id )
VALUES ('".$affiliate_id."', '".$product."', '".$comission."', '".$sale_amount."', '".$net_earnings."', '".$recurring_period."', '".$recurring_fee."', '".$datetime."', '".$stop_recurring."', '".$country."', '".$datetime."', '".$void."','".$refund."','".$attendee_id."')";
...
//$mysqli->close();
	...
	//	$transaction_id ='';
		...
		//$transaction_id = $insert->insert_id;
